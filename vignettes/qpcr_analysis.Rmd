...
res <- pcr_assess(ct3,
                  ...
                  method = 'efficiency')
knitr::kable(res, 
             caption = '\\label{table:table7} Amplificaion efficiency of c-myc')
```
...
The output of `pcr_analyze` is 8 columns; contains the calculatations of each gene in each group and the error terms (Table \ref{table:table9}). This analysis uses the default `mode`, 'separate_tube' as the input dataset came for an experiment where the target `r names(ct1)[1]` and the control gene `r names(ct1)[2]` were ran in separate tubes.  

In contrast, the `ct2` dataset, also shown in Table \ref{table3}, came from an identical experment except the samples were run in the same tube. So the followin analysis invokes a different `mode`, 'same_tube'.  
...
To apply this method to the `ct1` dataset (Table \ref{table2}). We used the variables `slope` and `intercept` that were calculated earlier using the `ct3` dataset (Table \ref{table:table8}).  
...
The output is similar to that descriped earlier (Table \ref{table:table13}).  
